	...
	private void reheapDown(MyNode<Ttype> nodeTemp) {
		if(nodeTemp != null) {
			...
			if(nodeTemp.getLeftChNode() != null 
				...
				if( ((Comparable)nodeTemp.getElement()).compareTo(nodeTemp.getLeftChNode().getElement()) < 0) {
			...
			if(((Comparable)nodeTemp.getLeftChNode().getElement()).compareTo(nodeTemp.getRightChNode().getElement()) > 0) {
				//vai kreisais bērns ir lielaķs par pasu bloka vērtību
				if( ((Comparable)nodeTemp.getLeftChNode().getElement()).compareTo(nodeTemp.getElement()) > 0) {
					swap(nodeTemp, nodeTemp.getLeftChNode());
			...
			else
			{
				...
				if( ((Comparable)nodeTemp.getRightChNode().getElement()).compareTo(nodeTemp.getElement()) > 0) {
